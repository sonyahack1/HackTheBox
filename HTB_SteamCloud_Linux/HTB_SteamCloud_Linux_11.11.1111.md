...
> Во время прохождения данной машины были обнаружены небезопасные API-интерфейсы, допускающие инъекции команд, что обеспечило первоначальный доступ в систему под пользователем `www-data`.
> В системе в файле `.env` были обнаружены в открытом виде учетные данные для пользователя `admin`, что позволило выполнить технику горизонтальной эскалации привилегий. Завершающим этапом
> стало использование уязвимости в ядре Linux `(CVE-2023-0386)`, что привело к полной компрометации системы. Машина `TwoMillion` демонстрирует важность защиты API, контроля над конфигурационными
> файлами и своевременного обновления ядра.
